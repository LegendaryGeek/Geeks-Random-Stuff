...
public class GUIFoodInfuser extends GuiContainer
{
    ...
    {
        String s = this.tileFoodInfuser.getDisplayName().getUnformattedText();
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        ...
        this.mc.getTextureManager().bindTexture(FoodInfuser_GUI_TEXTURES);
        int i = (this.width - this.xSize) / 2;
        int j = (this.height - this.ySize) / 2;
        this.drawTexturedModalRect(i, j, 0, 0, this.xSize, this.ySize);

        if (TileEntityFurnace.isBurning(this.tileFoodInfuser))
        {
    ...
    {
        int i = this.tileFoodInfuser.getField(2);
        int j = this.tileFoodInfuser.getField(3);
        return j != 0 && i != 0 ? i * pixels / j : 0;
    }

    private int getBurnLeftScaled(int pixels)
    {
        int i = this.tileFoodInfuser.getField(1);
        ...
        return this.tileFoodInfuser.getField(0) * pixels / i;
    }
}